...
type Props = {
  ...
  onContinue: () => void
  options?: BouqetTypeOption[]
  isLoading?: boolean
  message?: string
}

export default function BouqetsType({ selected, onSelect, onContinue, options = [], isLoading = false, message = '' }: Props) {
  const [brokenImages, setBrokenImages] = useState<Record<string, boolean>>({})

  const showMessage = Boolean(message) || (!isLoading && options.length === 0)
  const messageText = message || 'No bouquet types available.'
