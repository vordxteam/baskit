...
export default function DecorateSection({ decoration, onDecorChange, onBack, onContinue }: Props) {
  return (
    <section id="decorate" className="scroll-mt-24">
      <h2 className="text-[28px] sm:text-[32px] tobia-normal text-[#252525] mb-8">
        Decorate your Baskit
      </h2>
      ...
      <div className="flex flex-col sm:flex-row gap-10 sm:gap-16">
        <ColorPicker
          ...
          onSelect={(v) => onDecorChange('basketColor', v)}
        />
        <ColorPicker
          label="Net color"
          options={netColors}
          selected={decoration.netColor}
          onSelect={(v) => onDecorChange('netColor', v)}
        />
        <ColorPicker
          label="Ribbon color"
