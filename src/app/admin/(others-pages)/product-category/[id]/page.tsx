...
interface ProductTypeDetailsResponse {
  success?: boolean;
  message?: string;
  data?: ProductTypeDetails;
}

export default function ViewProductCategoryPage() {
  const params = useParams();
  const typeId = Array.isArray(params?.id) ? params.id[0] : params?.id;
  const [data, setData] = useState<ProductTypeDetails | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchTypeById = async () => {
      try {
        const res = await productApi.getProductTypeById<ProductTypeDetailsResponse>(typeId);
        setData(res.data ?? null);
      } catch (error) {
        console.error("Fetch product category by id failed:", error);
      }
    };
    ...
    <div className="mx-auto max-w-3xl">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-semibold text-gray-800 dark:text-white/90">{data?.name || "Product Category"}</h1>
              <Badge size="sm" color={data?.is_active ? "success" : "error"}>
                {data?.is_active ? "Active" : "Inactive"}
              </Badge>
            </div>
            <p className="text-sm text-gray-500 dark:text-gray-400">ID #{typeId}</p>
          </div>
        </div>

        <Link
          href={`/admin/product-category/${typeId}/edit`}
      ...
      <div className="mb-5 rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
        <div className="border-b border-gray-100 px-5 py-4 dark:border-white/[0.05]">
          <h2 className="text-sm font-semibold text-gray-700 dark:text-white/80">Product Category Details</h2>
        </div>
        <div className="divide-y divide-gray-100 dark:divide-white/[0.05]">
          ...
          <div className="grid grid-cols-3 px-5 py-4">
            <span className="text-sm text-gray-500 dark:text-gray-400">Sort Order</span>
            <span className="col-span-2 text-sm text-gray-700 dark:text-white/70">{data?.sort_order ?? "-"}</span>
          </div>
          <div className="grid grid-cols-3 px-5 py-4">
