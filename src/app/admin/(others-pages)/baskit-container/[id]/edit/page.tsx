...
export default function EditProductContainerPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const containerData = await productContainerApi.getContainer(id);
        ...
        if (containerData) {
          ...
          setImagePreview(containerData.image || null);
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsLoading(true);
      const productContainerApi = new ProductContainer();
      await productContainerApi.updateContainer(id, form);
      router.push("/admin/baskit-container");
    } catch (err) {
      console.error("Error updating product container:", err);
      setError("Failed to update product container");
    } finally {
      setIsLoading(false);
    }
  };
