...
export default function CategoryPage() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      try {
        const res = await productApi.getCategory<CategoriesApiResponse>();
        console.log("Categories response:", res);

        const mappedCategories: Category[] = (res.data || []).map((item) => ({
          id: item.id,
          name: item.name,
          slug: item.slug,
  ...
  const handleDeleteConfirm = async () => {
    if (!deleteModal.id || deletingCategoryId) {
      return;
    }

    setApiError("");
    setDeletingCategoryId(deleteModal.id);

    try {
      await productApi.deleteCategory(deleteModal.id);
    ...
    <>
      ...
      <div className="rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
        ...
        <div className="flex flex-col gap-3 border-b border-gray-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between dark:border-white/[0.05]">
          <div>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Categories</h3>
                  ...
                  <TableRow key={item.id}>
                    <TableCell className="px-5 py-4 text-start">
                      <div className="min-w-0">
                        ...
                        <span className="block text-theme-xs text-gray-500 dark:text-gray-400 truncate max-w-[220px]">
                          {item.description}
                        </span>
                      </div>
                    </TableCell>
            ...
            >
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <div>
                  ...
                  <p className="text-sm font-mono font-medium text-gray-800 dark:text-white/90">{item.slug}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Products</p>
                  <p className="text-sm font-medium text-gray-800 dark:text-white/90">{item.productCount}</p>
                </div>
                <div>
        ...
        {!loading && categories.length > 0 && (
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-gray-500 dark:text-gray-400">
              ...
              of{" "}
              <span className="font-medium text-gray-700 dark:text-white/90">{categories.length}</span>{" "}
              categories
            </p>
      ...
      <ConfirmationModal
        ...
        title="Delete Category"
        message="Are you sure you want to delete this category? Products in this category may be affected. This action cannot be undone."
        confirmLabel="Delete"
        cancelLabel="Cancel"
        variant="danger"
      />
