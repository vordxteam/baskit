...
import { UpdateProductSizeRequest, ProductSize as ProductSizeModel } from "@/api/productSize/types";
import { ProductApi } from "@/api/products";

export default function EditProductSizePage() {
  ...
  const [form, setForm] = useState<UpdateProductSizeRequest>({
    product_type_id: "",
    size_code: "",
    label: "",
    min_items: 0,
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch product types
        const typesResponse = await new ProductApi().getProductTypes();
        const types =
        ...
        // Fetch size details
        const sizeResponse = await new ProductSize().getSizeById(sizeId);
        const size = sizeResponse.data as ProductSizeModel;
        setForm({
          product_type_id: size.product_type_id,
          size_code: size.size_code,
          label: size.label,
          min_items: size.min_items,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await new ProductSize().updateSize(sizeId, form);
      router.push(`/admin/product-sizes/${sizeId}`);
      router.refresh();
    } catch (err) {
      console.error("Error updating product size:", err);
      setError("Failed to update product size");
    } finally {
  ...
  return (
    <div className="px-4 py-8 sm:px-6">
      ...
      <div className="max-w-5xl m-auto rounded-xl border border-gray-200 bg-white p-6 dark:border-white/5 dark:bg-white/3">
