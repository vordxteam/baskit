...
export default function ProductSubTypeTable() {
  ...
  useEffect(() => {
    ...
    const fetchContainers = async () => {
      ...
      try {
        ...
        const data = await productSubTypeApi.getSubtypes();
        ...
        const mapped = data.map((item: ProductSubTypeData) => ({
          id: item.id || '',
          name: item.name,
  ...
  const handleDeleteConfirm = () => {
    ...
    const runDelete = async () => {
      try {
        ...
        setFetchError("");
        const productSubTypeApi = new ProductSubType();
        await productSubTypeApi.deleteSubtypes(containerId);
    ...
    <>
      <div className="rounded-xl border border-gray-200 bg-white dark:border-white/5 dark:bg-white/3">
        ...
        <div className="flex flex-col gap-3 border-b border-gray-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between dark:border-white/5">
          <div>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Product Subtypes</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400">Manage your product subtypes</p>
          </div>

          {/* Add Subtype Button */}
          <Link
            href="/admin/product-subtype/add"
        ...
        {isLoading && (
          <div className="p-10 text-center">
            <p className="text-sm text-gray-500 dark:text-gray-400">Loading subtypes...</p>
          </div>
        )}

        {!isLoading && containers.length === 0 && (
          <div className="p-10 text-center">
            <h4 className="text-base font-medium text-gray-800 dark:text-white/90">No subtypes found</h4>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Create your first subtype to get started.</p>
          </div>
        )}
                      ...
                      <TableRow key={item.id}>
                        ...
                        <TableCell className="px-5 py-4">
                          <div className="flex items-center justify-end gap-2">
                            {/* View */}
                            <Link
                              href={`/admin/product-subtype/${item.id}`}
                            ...
                            {/* Edit */}
                            <Link
                              href={`/admin/product-subtype/${item.id}/edit`}
                ...
                <div key={item.id} className="rounded-xl border border-gray-200 bg-white p-4 dark:border-white/5 dark:bg-white/3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-1">
                      <Link href={`/admin/product-subtype/${item.id}`} className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-blue-50 hover:text-blue-600 transition-colors">
                        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.964-7.178z" />
                          <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                        </svg>
                      </Link>
                      <Link href={`/admin/product-subtype/${item.id}/edit`} className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-amber-50 hover:text-amber-600 transition-colors">
                        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <ConfirmationModal
        ...
        title="Delete Subtype"
        message="Are you sure you want to delete this subtype? This action is permanent and cannot be undone."
