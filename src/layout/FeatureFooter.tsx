import React from "react";
import Link from "next/link";

const footerLinks = [
    { label: "Home", href: "/" },
    { label: "About", href: "/about" },
    { label: "Shop", href: "/shop" },
    { label: "Categories", href: "/categories" },
    { label: "Reviews", href: "/reviews" },
];

const contactDetails = [
    { label: "Phone", value: "+92345678987", href: "tel:+92345678987" },
    { label: "Email", value: "baskit@gmail.com", href: "mailto:baskit@gmail.com" },
    { label: "Address", value: "145G Block, DHA Phase 1, Lahore" },
];

const FeatureFooter = () => {
    return (
        <footer className="bg-[#202020] text-white">
            <div className="mx-auto max-w-[1440px] px-5 sm:px-10 pb-10 pt-20">
                <div className="flex flex-col gap-10 lg:flex-row lg:items-start lg:justify-between">
                    {/* Left nav */}
                    <div className="w-full lg:max-w-[180px]">
                        <nav>
                            <ul className="space-y-3">
                                {footerLinks.map((item) => (
                                    <li key={item.label}>
                                        <Link
                                            href={item.href}
                                            className="text-[12px] uppercase tracking-[0.04em] text-[#F2EEE899] transition hover:text-white"
                                        >
                                            {item.label}
                                        </Link>
                                    </li>
                                ))}
                            </ul>
                        </nav>
                    </div>

                    {/* Center support content */}
                    <div className="w-full lg:max-w-[420px]">
                        <h2 className="text-[28px] leading-8 text-white tobia-normal">
                           +92 345 789875
                        </h2>

                        <p className="mt-2 text-[14px] leading-5 text-[#F2EEE8CC]">
                            24/7 Personal Support
                        </p>

                        <p className="mt-5 max-w-[413px] text-[14px] leading-5 text-[#F2EEE8CC]">
                            We&apos;re always here to help, whenever you need us. Our team is
                            available around the clock to assist with any questions or
                            concerns, ensuring a seamless and worry-free experience.
                        </p>
                    </div>

                    {/* Right contact details */}
                    <div className="w-full lg:max-w-[340px]">
                        <div className="space-y-4">
                            {contactDetails.map((item) => (
                                <div
                                    key={item.label}
                                    className="grid grid-cols-[88px_1fr] gap-4 sm:grid-cols-[100px_1fr]"
                                >
                                    <span className="text-[12px] uppercase tracking-[0.04em] text-white/55">
                                        {item.label}
                                    </span>

                                    {item.href ? (
                                        <Link
                                            href={item.href}
                                            className="text-[12px] uppercase tracking-[0.03em] text-white/85 transition hover:text-white sm:text-[13px] flex items-end justify-end"
                                        >
                                            {item.value}
                                        </Link>
                                    ) : (
                                        <p className="text-[12px] uppercase tracking-[0.03em] text-white/85 sm:text-[13px]">
                                            {item.value}
                                        </p>
                                    )}
                                </div>
                            ))}
                        </div>
                    </div>
                </div>

                {/* Bottom row */}
                <div className="mt-10 flex flex-col gap-6 border-white/10 pt-2 sm:mt-12 md:flex-row md:items-end md:justify-between">
                    <p className="text-[12px] text-white/45">
                        @2026 Baskit. All rights reserved.
                    </p>

                    <div className="flex items-center md:justify-end">
                        <div className="flex overflow-hidden border border-white/15">
                            <Link
                                href="https://www.facebook.com"
                                aria-label="Facebook"
                                className="flex h-11 w-11 items-center justify-center border-r border-white/15 text-white/85 transition hover:bg-white hover:text-[#202020]"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                    <path d="M22 12C22 6.48 17.52 2 12 2C6.48 2 2 6.48 2 12C2 16.84 5.44 20.87 10 21.8V15H8V12H10V9.5C10 7.57 11.57 6 13.5 6H16V9H14C13.45 9 13 9.45 13 10V12H16V15H13V21.95C18.05 21.45 22 17.19 22 12Z" fill="#FFFEF2" />
                                </svg>
                            </Link>

                            <Link
                                href="https://www.instagram.com"
                                aria-label="Instagram"
                                className="flex h-11 w-11 items-center justify-center border-r border-white/15 text-white/85 transition hover:bg-white hover:text-[#202020]"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                    <path d="M13.0276 2C14.1526 2.003 14.7236 2.009 15.2166 2.023L15.4106 2.03C15.6346 2.038 15.8556 2.048 16.1226 2.06C17.1866 2.11 17.9126 2.278 18.5496 2.525C19.2096 2.779 19.7656 3.123 20.3216 3.678C20.8303 4.17773 21.2238 4.78247 21.4746 5.45C21.7216 6.087 21.8896 6.813 21.9396 7.878C21.9516 8.144 21.9616 8.365 21.9696 8.59L21.9756 8.784C21.9906 9.276 21.9966 9.847 21.9986 10.972L21.9996 11.718V13.028C22.002 13.7574 21.9944 14.4868 21.9766 15.216L21.9706 15.41C21.9626 15.635 21.9526 15.856 21.9406 16.122C21.8906 17.187 21.7206 17.912 21.4746 18.55C21.2238 19.2175 20.8303 19.8223 20.3216 20.322C19.8219 20.8307 19.2171 21.2242 18.5496 21.475C17.9126 21.722 17.1866 21.89 16.1226 21.94L15.4106 21.97L15.2166 21.976C14.7236 21.99 14.1526 21.997 13.0276 21.999L12.2816 22H10.9726C10.2429 22.0026 9.51312 21.9949 8.78359 21.977L8.58959 21.971C8.3522 21.962 8.11487 21.9517 7.87759 21.94C6.81359 21.89 6.08759 21.722 5.44959 21.475C4.78242 21.2241 4.17804 20.8306 3.67859 20.322C3.16954 19.8224 2.7757 19.2176 2.52459 18.55C2.27759 17.913 2.10959 17.187 2.05959 16.122L2.02959 15.41L2.02459 15.216C2.00616 14.4868 1.99782 13.7574 1.99959 13.028V10.972C1.99682 10.2426 2.00416 9.5132 2.02159 8.784L2.02859 8.59C2.03659 8.365 2.04659 8.144 2.05859 7.878C2.10859 6.813 2.27659 6.088 2.52359 5.45C2.77529 4.7822 3.16982 4.17744 3.67959 3.678C4.17875 3.16955 4.78278 2.77607 5.44959 2.525C6.08759 2.278 6.81259 2.11 7.87759 2.06C8.14359 2.048 8.36559 2.038 8.58959 2.03L8.78359 2.024C9.51278 2.00623 10.2422 1.99857 10.9716 2.001L13.0276 2ZM11.9996 7C10.6735 7 9.40174 7.52678 8.46406 8.46447C7.52638 9.40215 6.99959 10.6739 6.99959 12C6.99959 13.3261 7.52638 14.5979 8.46406 15.5355C9.40174 16.4732 10.6735 17 11.9996 17C13.3257 17 14.5974 16.4732 15.5351 15.5355C16.4728 14.5979 16.9996 13.3261 16.9996 12C16.9996 10.6739 16.4728 9.40215 15.5351 8.46447C14.5974 7.52678 13.3257 7 11.9996 7ZM11.9996 9C12.3936 8.99993 12.7837 9.07747 13.1477 9.22817C13.5117 9.37887 13.8424 9.5998 14.1211 9.87833C14.3997 10.1569 14.6207 10.4875 14.7715 10.8515C14.9224 11.2154 15 11.6055 15.0001 11.9995C15.0002 12.3935 14.9226 12.7836 14.7719 13.1476C14.6212 13.5116 14.4003 13.8423 14.1218 14.121C13.8432 14.3996 13.5126 14.6206 13.1486 14.7714C12.7847 14.9223 12.3946 14.9999 12.0006 15C11.2049 15 10.4419 14.6839 9.87927 14.1213C9.31666 13.5587 9.00059 12.7956 9.00059 12C9.00059 11.2044 9.31666 10.4413 9.87927 9.87868C10.4419 9.31607 11.2049 9 12.0006 9M17.2506 5.5C16.9191 5.5 16.6011 5.6317 16.3667 5.86612C16.1323 6.10054 16.0006 6.41848 16.0006 6.75C16.0006 7.08152 16.1323 7.39946 16.3667 7.63388C16.6011 7.8683 16.9191 8 17.2506 8C17.5821 8 17.9001 7.8683 18.1345 7.63388C18.3689 7.39946 18.5006 7.08152 18.5006 6.75C18.5006 6.41848 18.3689 6.10054 18.1345 5.86612C17.9001 5.6317 17.5821 5.5 17.2506 5.5Z" fill="#FFFEF2" />
                                </svg>
                            </Link>

                            <Link
                                href="https://www.tiktok.com"
                                aria-label="TikTok"
                                className="flex h-11 w-11 items-center justify-center text-white/85 transition hover:bg-white hover:text-[#202020]"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M16 21.75C17.525 21.75 18.9875 21.1442 20.0659 20.0659C21.1442 18.9875 21.75 17.525 21.75 16V8C21.75 6.47501 21.1442 5.01247 20.0659 3.93414C18.9875 2.8558 17.525 2.25 16 2.25H8C6.47501 2.25 5.01247 2.8558 3.93414 3.93414C2.8558 5.01247 2.25 6.47501 2.25 8V16C2.25 17.525 2.8558 18.9875 3.93414 20.0659C5.01247 21.1442 6.47501 21.75 8 21.75H16ZM13.711 5.763C13.6544 5.59444 13.5398 5.45147 13.3876 5.35959C13.2353 5.26771 13.0554 5.2329 12.8799 5.26137C12.7044 5.28985 12.5447 5.37974 12.4293 5.51503C12.314 5.65032 12.2504 5.8222 12.25 6V15C12.25 15.445 12.118 15.88 11.8708 16.25C11.6236 16.62 11.2722 16.9084 10.861 17.0787C10.4499 17.249 9.9975 17.2936 9.56105 17.2068C9.12459 17.1199 8.72368 16.9057 8.40901 16.591C8.09434 16.2763 7.88005 15.8754 7.79323 15.439C7.70642 15.0025 7.75097 14.5501 7.92127 14.139C8.09157 13.7278 8.37996 13.3764 8.74997 13.1292C9.11998 12.882 9.55499 12.75 10 12.75C10.1989 12.75 10.3897 12.671 10.5303 12.5303C10.671 12.3897 10.75 12.1989 10.75 12C10.75 11.8011 10.671 11.6103 10.5303 11.4697C10.3897 11.329 10.1989 11.25 10 11.25C9.25832 11.25 8.5333 11.4699 7.91661 11.882C7.29993 12.294 6.81928 12.8797 6.53545 13.5649C6.25162 14.2502 6.17736 15.0042 6.32206 15.7316C6.46675 16.459 6.8239 17.1272 7.34835 17.6517C7.8728 18.1761 8.54098 18.5332 9.26841 18.6779C9.99584 18.8226 10.7498 18.7484 11.4351 18.4645C12.1203 18.1807 12.706 17.7001 13.118 17.0834C13.5301 16.4667 13.75 15.7417 13.75 15V8.458C14.517 9.17 15.597 9.75 17 9.75C17.1989 9.75 17.3897 9.67098 17.5303 9.53033C17.671 9.38968 17.75 9.19891 17.75 9C17.75 8.80109 17.671 8.61032 17.5303 8.46967C17.3897 8.32902 17.1989 8.25 17 8.25C16.028 8.25 15.289 7.85 14.741 7.331C14.181 6.799 13.843 6.158 13.711 5.763Z" fill="#FFFEF2" />
                                </svg>
                            </Link>
                        </div>
                    </div>
                </div>
            </div>
        </footer>
    );
};

export default FeatureFooter;